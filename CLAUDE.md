# exec-sandbox — Claude Code layer

The canonical, harness-neutral briefing for this repo is **`AGENTS.md`**. Read it
first — it holds project context, commands, the invariants and v1 contract, the task
...
spec-coverage-check, protect-checkout, config-protection.

## Skills

- **code-scanner** — scan any code/package/deps this block will build on or run
  before trusting them. Trigger: "scan this repo for malware"
- **code-review** — review diffs before merge, especially anything touching the
  egress proxy or bwrap argv. Trigger: `/code-review`
- **deep-research** — when adding an isolation backend (gVisor, Firecracker, Kata),
  survey prior art / build-vs-adopt first. Trigger: "deep research on <X>"
