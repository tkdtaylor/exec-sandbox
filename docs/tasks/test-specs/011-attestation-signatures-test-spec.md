...
**ADR:** the attestation ADR (next available ADR number — ADR numbers are sequential-by-creation, not bound to the task ID; **required before implementation** — settles the signing key / trust root and the attested-fields canonical encoding; see the task's readiness note).
**Written:** 2026-06-20
...
**Open decision — settle in the attestation ADR BEFORE writing code (precondition):**
...
2. **Consumer contract check (load-bearing).** The attestation is presented to **vault** via
   ...
   contract cannot be confirmed from the available block contracts, the attestation ADR records the assumption
...
whichever option the attestation ADR picks. If the attestation ADR picks the host-key option, the `sandbox_identity` shape
...
|--------|-------------|-----------|----------|
| REQ-011-01 | `sandbox_identity.attestation` is a cryptographic **signature** over the documented attested fields (per the attestation ADR), not random bytes; it is minted per run | TC-011-01 | ✅ |
| REQ-011-02 | The attestation **verifies** against the public key (the in-identity public key for self-attestation, or the trusted host key per the attestation ADR) over the reconstructed preimage of the attested fields | TC-011-02 | ✅ |
...
| REQ-011-04 | The attestation is presented to `vault.inject` in the `sandbox_identity` exactly as before structurally (the inject request still carries `{sandbox_id, attestation}` plus any added field the attestation ADR introduces, e.g. a public key); the IPC contract is not broken | TC-011-04 | ✅ |
...
- [ ] **BLOCKER:** the attestation ADR written settling the trust root AND the vault-consumer contract confirmed
...
- **Expected:** `attestation` is a signature of the expected length/encoding for the chosen scheme
  (ed25519 ⇒ 64-byte signature, hex/base64 per the attestation ADR) over the attested-fields preimage — not a
...
- **Type:** unit
- **Input:** a minted `sandbox_identity`; reconstruct the attested-fields preimage per the attestation ADR;
...
- **Expected:** the recorded `vault.inject` request carries
  ...
  the attestation-ADR-added field (e.g. `attestation_pubkey`) is present; the JSON-line IPC contract is intact
...
- **Expected:** the `sandbox_identity` section documents the signed attestation — its shape, the
  attested fields, and the verify-key source (per the attestation ADR) — and the
...
- [ ] the attestation ADR written AND vault-consumer contract confirmed (or assumption recorded + blocked)
...
- TC-011-03's tamper cases must reconstruct the **exact** preimage the attestation ADR specifies, or the
