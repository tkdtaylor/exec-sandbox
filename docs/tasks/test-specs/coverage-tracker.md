...
| 004 | `FileRead{paths}` read-only host mounts + payload PATH/env provisioning (bwrap + gVisor) | [`004-toolchain-mount-and-path-test-spec.md`](004-toolchain-mount-and-path-test-spec.md) | TC-001..011 planned (not yet written) | ❌ Not started | pending — backlog task; implement `FileRead{paths}` (read-only) + `run.env`/`run.path` PATH provisioning so a caller can mount a read-only host toolchain and the payload finds it on PATH. Target L5/L6 on a bwrap+runsc host (tool resolves via `command -v`, read-only write fails, netns stays unshared). ADR 005 to be written during implementation. |
