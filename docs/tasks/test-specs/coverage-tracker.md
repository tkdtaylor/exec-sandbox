...
| 003 | Writable working-directory mount (`run.workdir` → `/work` rw, cwd=/work) on bwrap + gVisor | [`003-writable-workdir-mount-test-spec.md`](003-writable-workdir-mount-test-spec.md) | TC-001..010 written + passing | 🟡 code merged | L2 (pending spec-verifier + L5/L6 promotion): `go test -count=1 ./...` 26 PASS / 0 SKIP on a bwrap+runsc host |
