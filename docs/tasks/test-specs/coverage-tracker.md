...
| 008 | Snapshot/restore for clean-slate sandbox reuse — leak-proof reset behind the unchanged one-shot `run()` | [`008-snapshot-restore-test-spec.md`](008-snapshot-restore-test-spec.md) | TC-008-01..07 written + passing | 🟡 code merged | L6 (executor, pending spec-verifier): `go test -count=1 ./...` → `ok github.com/tkdtaylor/exec-sandbox` 0 FAIL; all 7 task-008 TCs PASS / 0 SKIP on a bwrap host. The two L6 leak tests RAN un-skipped: `TestSecondRunCannotSeeFirstRunFiles_Bwrap` — run 1 writes `/work/leak.txt`, after host-side restore run 2 prints `CLEAN` (not `LEAKED`); `TestOneShotRunUnchanged_Bwrap` — one-shot run returns 200 via proxy + full sandbox_status schema (byte-for-byte unchanged). Load-bearing assertions: `TestNoStateLeaksAcrossRestore` (**restored == fresh** diff on surface + payload.sh + creds) and `TestNoCredentialLeaksAcrossRestore` (credential map empty after restore). Restored argv keeps `--unshare-all`, no `--share-net`, same fresh `/proxy.sock`. + L3 fitness F-010. ADR 009 written (four open-question dispositions); spec (behaviors B-012 / fitness F-010) + diagrams updated in place. spec-verifier APPROVE pending before ✅. |
