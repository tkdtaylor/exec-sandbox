...
| 007 | Per-HTTP-verb allowlist enforcement in the egress proxy (allowed verb forwards; disallowed → 403, no upstream) | [`007-proxy-verb-allowlist-test-spec.md`](007-proxy-verb-allowlist-test-spec.md) | TC-007-01..08 written + passing | 🟡 code merged | L3: `go test -count=1 ./...` → `ok github.com/tkdtaylor/exec-sandbox` 0 FAIL; verb integration TCs (TC-007-05/06/08-integ) RAN under bwrap (not skipped) — allowed verb 200 + 1 origin hit, disallowed POST 403 + **0 origin hits**; unit TCs prove `blocked-by-method` vs `blocked-by-allowlist`, case-insensitive match, empty-set=unconstrained. + L3 fitness F-009. Awaiting spec-verifier APPROVE + L5/L6 before ✅. |
