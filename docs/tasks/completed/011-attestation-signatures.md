...
**ADR:** the next available ADR number (attestation ADR) — **REQUIRED BEFORE IMPLEMENTATION** (see Readiness).
...
first, in the attestation ADR (the next available ADR number):
...
2. **Vault consumer contract check (load-bearing).** The attestation is handed to **vault** via
   ...
   (`interface-contracts.md`, the vault block repo), the attestation ADR records
...
**Verdict for the planner/operator:** ready to execute **only after** the attestation ADR is written and the
...
`attestation` as opaque/self-verifying (no host-key requirement) → the attestation ADR picks ephemeral ed25519
...
(Implementation begins **after** the attestation ADR resolves the trust root and the vault contract.)
...
- **Replace the random-bytes attestation** (`run.go:67`, the `randHex(16)` for `attestation`) with a
  **signed attestation**: a signature over the documented attested fields (at minimum `sandbox_id`;
  likely a timestamp/nonce for replay resistance — fixed in the attestation ADR) using the chosen key, in a
...
- **Preserve the IPC contract.** `vault.inject` still receives
  `sandbox_identity:{sandbox_id, attestation, …}` (plus any the attestation-ADR-added field like the public
...
Out of scope: a persistent / CA-backed key hierarchy beyond what the attestation ADR chooses; attesting the
payload contents or the tier (attest the identity, not the workload — note as the attestation ADR's reopening
...
- **The attestation ADR written FIRST** (next available ADR number — not bound to the task ID): settles the trust root (ephemeral ed25519 self-attestation vs host
...
- The attestation ADR written; vault-consumer contract confirmed (or assumption recorded + the task was
