...
| `run.profile.capabilities[FileRead].paths` | `[string]` (abs host paths) | `[]` | **Read-only host mounts** (ADR 005). Under bubblewrap/gVisor each path is bind-mounted **read-only** at the **same** path inside the sandbox; **under Firecracker each path is a separate read-only ext4 block-device drive** (`/dev/vdd…`, `is_read_only:true`), seeded from the host path (copy-in via `mkfs.ext4 -d`), exposed by the guest init read-only at the same host path (device→mountpoint mapping on the `exec_sandbox.fileread=…` kernel cmdline) — a guest write fails and the host file is never modified/created; **no copy-out** (B-016). Multiple `FileRead` entries union their paths. Validated before spawn (each must be absolute + exist, else a hard `{error}`). The read-**write** host dir is `run.workdir`, not `FileRead`. |
| `run.profile.limits` | object | — | **Enforced** resource caps (ADR 003). See the per-field table below. |
| `run.workdir` | string (host path) | `""` | **Writable working directory** (ADR 004). Non-empty → under bubblewrap/gVisor the host dir is bind-mounted **read-write** at `/work`; **under Firecracker it is a writable ext4 block-device drive** (`/dev/vdc`, `is_read_only:false`), seeded from the host dir (copy-in via `mkfs.ext4 -d`), mounted read-write at `/work` by the guest init, with the guest's writes copied back into the host dir **at teardown** via `debugfs rdump` (copy-out — both unprivileged, no root loop-mount; ADR 010 Q2). The payload's cwd is `/work`; validated before spawn (must be an existing directory, else a hard `{error}`). Empty → no `/work` mount (backward compatible). The **one** writable host surface — system dirs, the payload drive, and every FileRead drive stay read-only (a config-level guard enforces exactly one writable drive); the network stays unshared. Persist timing differs by tier: bubblewrap/gVisor persist **live**, Firecracker persists **at teardown** (B-016). |
...
| Runtime dependency (Tier-3) | `firecracker` on `PATH` + rw `/dev/kvm` + `mkfs.ext4` (and `debugfs` for `/work` copy-out) | Firecracker tier (B-015/B-016). The host user needs rw on `/dev/kvm` (the `kvm` group or an equivalent ACL) — **no root, no setuid, no jailer, no loop-mount**. `mkfs.ext4` builds the rootfs/payload/`/work`/FileRead drives (copy-in); `debugfs` reads the `/work` drive back at teardown (copy-out) — its absence makes copy-out a best-effort no-op (the host workdir keeps its pre-run state), not a hard error. Absence of `firecracker`/`/dev/kvm`/`mkfs.ext4` is a spawn error (127), never a fall-back; integration tests skip without `/dev/kvm`/`firecracker`. |
