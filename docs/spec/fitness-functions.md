...
**Last updated:** 2026-06-20 (task 009: F-001/F-002/F-004 wired — `fitness-<id>` Make targets + `fitness:` umbrella added)
...
```bash
make fitness          # run all 9 block-severity fitness functions (F-001, F-002, F-004..F-010)
make fitness-<rule>   # run one rule by name (e.g. make fitness-no-share-net)
make fitness-no-deps  # run F-003 (warn) in isolation — excluded from the umbrella
```

The `fitness:` umbrella is a `.PHONY` Make target whose prerequisites are **exactly** the 9
block-severity rules.  F-003 (`fitness-no-deps`) is `warn` severity and is excluded from the
umbrella — a warn-severity failure must never fail the umbrella.  Each `fitness-<id>` target
runs the `go test -run '<pattern>' ./...` command declared in its row below.

## Rules

| ID | Rule | Category | Asserts | Threshold | Check command | Severity | Status | Where enforced today |
|----|------|----------|---------|-----------|---------------|----------|--------|----------------------|
| F-001 | No shared network in any backend | security | No backend grants the sandbox a network namespace: the bwrap argv always carries `--unshare-all` and omits `--share-net`; the gVisor OCI spec declares an empty `network` namespace (no path) and `runsc` runs with `--network=none` | 0 violations | `make fitness-no-share-net` | block | **active** | `run.go` `bwrapArgv` hard-codes `--unshare-all`; `gvisor.go` `gvisorOCISpec` declares an empty netns and the argv adds `--network=none`. `TestFitnessNoShareNetPositive` asserts BOTH backends (bwrap + gVisor OCI spec); `TestFitnessNoShareNetNegative` proves the check rejects a mutated argv/spec (not a no-op) |
| F-002 | Proxy-mode credential never appears in sandbox env/args/stdout | security | A loaded credential value is never placed into the bwrap argv, the sandbox env, the payload, or the returned `stdout` | 0 leaks | `make fitness-cred-not-in-sandbox` | block | **active** | By construction: credentials live only in `EgressProxy.creds` (`proxy.go`) and are injected at the proxy edge. `TestFitnessCredNotInSandboxPositive` scans the bwrap argv, `--setenv` env pairs, and simulated stdout for a sentinel value; `TestFitnessCredNotInSandboxNegative` proves the scan catches a leak (not a no-op) |
