...
|----|------|----------|---------|-----------|---------------|----------|--------|----------------------|
| F-001 | No `--share-net` / network namespace in bwrap argv | security | The bubblewrap argv never grants the sandbox a network namespace; `--unshare-all` is always present | 0 violations | `make fitness-no-share-net` *(not yet wired)* | block | proposed | `run.go` `bwrapArgv` hard-codes `--unshare-all` and omits `--share-net`; no test asserts the *absence* yet |
