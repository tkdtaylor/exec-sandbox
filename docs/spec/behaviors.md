...
- **The only writable host surface is `run.workdir` at `/work`** (B-010), and only when the caller sets it. The rootfs, system dirs, and `/payload.sh` are always read-only; the writable surface never widens beyond the single caller-supplied directory, and it never opens an egress path (the network stays unshared).
