...
- **Response:** for each handle, exec-sandbox mints a `sandbox_identity` (`{sandbox_id, attestation}`) and calls `vault.inject(handle, sandbox_identity, mode)` over the `vault_socket`. The response's `delivery` field selects how the credential is handled, and the two modes are deliberately distinct:
...
- **Side effects:** the spawn argv and/or OCI `config.json` carry the caps (except `max_output_bytes`, which is host-side and touches neither); `sandbox_status.limits` records the applied values (including `max_output_bytes` and the `output_truncated` list of capped streams); the `exit` audit context carries `status`.
